...
Product scope does not replace repository scope. It aggregates repository evidence, evaluates selected cross-repository relationships, and generates one product-level improvement plan while preserving the exact member repository and path behind every finding.
...
V1 assembles the registered member snapshots into a temporary product evidence bundle and reuses the existing deterministic rule engine for relationship semantics. The initial cross-repository overlay is deliberately narrow:

- `IAP-C001` — consumer/storefront constraints must remain compatible with the canonical product contract;
- `IAP-CX01` — reconciliation ownership must not create conflicting authoritative control paths.
...
V1 does **not** blindly rerun every repository rule across the combined bundle, because doing so would double-count controls already evaluated at member scope.
...
When member `scan-result/v1` files are already available, product scope can be used without GitHub federation:
...
For each registered related repository, Guard:
...
4. resolves the repository's default branch to an immutable commit SHA;
5. scans that immutable snapshot; and
6. discards the token after the stateless invocation.
...
- are explicitly registered in the trusted manifest;
...
Related repositories
  → immutable current default-branch SHAs

Trusted product membership
  → triggering repository default branch
```
...
For a registered member, the Check adds a **Product Assessment** and **Product Improvement Plan** beneath the repository result.
